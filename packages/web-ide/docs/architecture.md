...
The standalone source repository's release tooling is a source/distribution
trust boundary, not runtime product code, and is not copied into Nova's
workspace mirror. In the standalone repository it is activated in Vite only by an absolute external provenance-output
