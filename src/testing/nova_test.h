...
// Results stream out as stdout markers that the IDE's Tests panel parses; the
// raw lines are not meant to be read by a human.
...
// `\n` and `\r` are escaped so a multi-line value can't masquerade as a new
// marker on a fresh line.
...
// Two-level indirection so `__COUNTER__` (or any expandable token) is
// expanded *once* by the caller before the inner paste pins it down.
// Without the indirection, `a##__COUNTER__` would yield `a__COUNTER__`
// literally — the preprocessor skips expansion of operands of ##.
...
// `__COUNTER__` is captured once at the macro use site and threaded through
// `ctr` so all three references (forward decl, registrar, definition) share
// the same numeric suffix. `static` gives each test internal linkage so
// counter collisions across translation units don't surface as link errors.
