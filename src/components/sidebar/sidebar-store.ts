...
const STORAGE_KEY = 'nova.sidebar'

export type SidebarView = 'assignment' | 'files'
...
function readStored(): Persisted {
    const fallback: Persisted = { activeView: 'files', collapsed: false }
    ...
    try {
        ...
        if (
            (parsed.activeView === 'assignment' || parsed.activeView === 'files') &&
