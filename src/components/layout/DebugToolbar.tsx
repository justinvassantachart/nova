...
export function DebugToolbar() {
    ...
    const historyGroup: ToolbarAction[] = [
        { label: 'Step back in history', icon: 'debug-step-back', tone: 'plain', disabled: !canStepBack, onClick: () => { host?.onEvent?.('debug_step_back', {}); stepBack() } },
        { label: 'Step forward in history', icon: 'debug-step-back', iconClassName: '-scale-x-100', tone: 'plain', disabled: !canStepForward, onClick: () => { host?.onEvent?.('debug_step_forward', {}); stepForward() } },
    ]
